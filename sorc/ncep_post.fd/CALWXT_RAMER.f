...
!   06-30-20  Bo CUI - Modernize inequality statements from Fortran 77 to 90
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!
...
!         IF (twq(I,J,k1)==twtop) GO TO 40        ! both equal twmelt, nothing h
          IF (twq(I,J,k1)<twmelt) exit loop40    ! cannot commence melting
          IF (twq(I,J,k1)==twtop) exit loop40     ! both equal twmelt, nothing h
...
!         IF (twq(I,J,k1)>twice) GO TO 40        ! cannot commence freezing
          IF (twq(I,J,k1)>twice) exit loop40     ! cannot commence freezing
...
!             IF (twq(I,J,k1)>twice) GO TO 40    ! cannot commence freezin
              IF (twq(I,J,k1)>twice) exit loop40 ! cannot commence freezin
              wgt1 = (twice-twq(I,J,k1)) / (twtop-twq(I,J,k1))
              dtavg = twmelt - (twq(I,J,k1)+twice) / 2
