...
!   -- rain
              ze_r = 1.e-35
              loop124: do
!             if (qqr(i,j,ll)<1.e-6) go to 124
              if (qqr(i,j,ll)<1.e-6) exit loop124
...
!   -- snow
              ze_s = 1.e-35
              loop125: do
!             if (qqs(i,j,ll)<1.e-6) go to 125
              if (qqs(i,j,ll)<1.e-6) exit loop125
...
!   -- graupel
              ze_g = 1.e-35
              loop126: do
!             if (qqg(i,j,ll)<1.e-6) go to 126
              if (qqg(i,j,ll)<1.e-6) exit loop126
...
!     
            ...
            .OR. (IGET(245)>0)  .or. IGET(464)>0 .or. IGET(467)>0) THEN
...
! CALCULATE Gust based on Ri PBL
      IF (IGET(245)>0 .or. IGET(464)>0 .or. IGET(467)>0) THEN
